/*
  ...
  1. New Tables
    - `campaign_sequences` - Campaign automation sequences
    - `conversation_history` - Message history between AI and leads
    - `training_resources` - AI training materials
    - `bookings` - Lead booking records
    - `lead_sequence_progress` - Progress tracking for sequences
    - `lead_activity_history` - Activity logs for leads
...
-- Add triggers for updated_at columns where needed
DO $$
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'uploaded_leads') THEN
    DROP TRIGGER IF EXISTS update_uploaded_leads_updated_at_trigger ON uploaded_leads;
